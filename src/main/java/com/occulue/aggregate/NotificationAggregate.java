...
public class NotificationAggregate {  
    ...
    public NotificationAggregate(CreateNotificationCommand command) throws Exception {
    	...
    	CreateNotificationEvent event = new CreateNotificationEvent(command.getNotificationId(), command.getTargets(), command.getObjectType(), command.getOperation());
    ...
    public void handle(UpdateNotificationCommand command) throws Exception {
    	...
    	UpdateNotificationEvent event = new UpdateNotificationEvent(command.getNotificationId(), command.getTargets(), command.getObjectType(), command.getOperation(), command.getNotifier());        
    ...
    public void handle(AssignNotifierToNotificationCommand command) throws Exception {
    	...
    	if (  notifier != null && notifier.getNotifierId() == command.getAssignment().getNotifierId() )
    		throw new ProcessingException( "Notifier already assigned with id " + command.getAssignment().getNotifierId() );  
    		
        apply(new AssignNotifierToNotificationEvent(command.getNotificationId(), command.getAssignment()));
    }
    ...
    public void handle(UnAssignNotifierFromNotificationCommand command) throws Exception {
    	...
    	if (  notifier == null )
    		throw new ProcessingException( "Notifier already has nothing assigned." );  
