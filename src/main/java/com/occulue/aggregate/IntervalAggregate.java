...
public class IntervalAggregate {  
    ...
    public IntervalAggregate(CreateIntervalCommand command) throws Exception {
    	...
    	CreateIntervalEvent event = new CreateIntervalEvent(command.getIntervalId(), command.getPayloads());
    ...
    public void handle(UpdateIntervalCommand command) throws Exception {
    	...
    	UpdateIntervalEvent event = new UpdateIntervalEvent(command.getIntervalId(), command.getPayloads(), command.getIntervalPeriod());        
    ...
    public void handle(AssignIntervalPeriodToIntervalCommand command) throws Exception {
        ...
        apply(new AssignIntervalPeriodToIntervalEvent(command.getIntervalId(), command.getAssignment()));
    }
    ...
    public void handle(AssignPayloadsToIntervalCommand command) throws Exception {
    	...
    	if ( payloads.contains( command.getAddTo() ) )
    		throw new ProcessingException( "Payloads already contains an entity with id " + command.getAddTo().getValuesMapId() );
    ...
    public void handle(RemovePayloadsFromIntervalCommand command) throws Exception {
    	...
    	if ( !payloads.contains( command.getRemoveFrom() ) )
    		throw new ProcessingException( "Payloads does not contain an entity with id " + command.getRemoveFrom().getValuesMapId() );
