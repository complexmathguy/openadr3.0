...
public class SubscriptionAggregate {  
    ...
    public SubscriptionAggregate(CreateSubscriptionCommand command) throws Exception {
    	...
    	CreateSubscriptionEvent event = new CreateSubscriptionEvent(command.getSubscriptionId(), command.getCreatedDateTime(), command.getModificationDateTime(), command.getClientName(), command.getTargets(), command.getObjectType());
    ...
    public void handle(UpdateSubscriptionCommand command) throws Exception {
    	...
    	UpdateSubscriptionEvent event = new UpdateSubscriptionEvent(command.getSubscriptionId(), command.getCreatedDateTime(), command.getModificationDateTime(), command.getClientName(), command.getProgram(), command.getObjectOperations(), command.getTargets(), command.getObjectType());        
    ...
    public void handle(AssignProgramToSubscriptionCommand command) throws Exception {
        ...
        apply(new AssignProgramToSubscriptionEvent(command.getSubscriptionId(), command.getAssignment()));
    }
    ...
    public void handle(AssignObjectOperationsToSubscriptionCommand command) throws Exception {
    	...
    	if ( objectOperations.contains( command.getAddTo() ) )
    		throw new ProcessingException( "ObjectOperations already contains an entity with id " + command.getAddTo().getObjectOperationId() );

    	apply(new AssignObjectOperationsToSubscriptionEvent(command.getSubscriptionId(), command.getAddTo()));
    }
    ...
    public void handle(RemoveObjectOperationsFromSubscriptionCommand command) throws Exception {
    	...
    	if ( !objectOperations.contains( command.getRemoveFrom() ) )
    		throw new ProcessingException( "ObjectOperations does not contain an entity with id " + command.getRemoveFrom().getObjectOperationId() );

        apply(new RemoveObjectOperationsFromSubscriptionEvent(command.getSubscriptionId(), command.getRemoveFrom()));
    }
