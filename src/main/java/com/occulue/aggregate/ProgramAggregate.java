...
public class ProgramAggregate {  
    ...
    public ProgramAggregate(CreateProgramCommand command) throws Exception {
    	...
    	CreateProgramEvent event = new CreateProgramEvent(command.getProgramId(), command.getCreatedDateTime(), command.getModificationDateTime(), command.getProgramName(), command.getProgramLongName(), command.getRetailerName(), command.getRetailerLongName(), command.getProgramType(), command.getCountry(), command.getPrincipalSubdivision(), command.getTimeZoneOffset(), command.getProgramDescriptions(), command.getBindingEvents(), command.getLocalPrice(), command.getTargets(), command.getObjectType());
    ...
    public void handle(UpdateProgramCommand command) throws Exception {
    	...
    	UpdateProgramEvent event = new UpdateProgramEvent(command.getProgramId(), command.getCreatedDateTime(), command.getModificationDateTime(), command.getProgramName(), command.getProgramLongName(), command.getRetailerName(), command.getRetailerLongName(), command.getProgramType(), command.getCountry(), command.getPrincipalSubdivision(), command.getTimeZoneOffset(), command.getProgramDescriptions(), command.getBindingEvents(), command.getLocalPrice(), command.getPayloadDescriptors(), command.getTargets(), command.getObjectType(), command.getIntervalPeriod());        
    ...
    public void handle(RemovePayloadDescriptorsFromProgramCommand command) throws Exception {
        ...
        apply(new RemovePayloadDescriptorsFromProgramEvent(command.getProgramId(), command.getRemoveFrom()));
    }
    @CommandHandler
    public void handle(AssignTargetsToProgramCommand command) throws Exception {
    	...
    	if ( targets.contains( command.getAddTo() ) )
    		throw new ProcessingException( "Targets already contains an entity with id " + command.getAddTo().getValuesMapId() );
    ...
    public void handle(RemoveTargetsFromProgramCommand command) throws Exception {
    	...
    	if ( !targets.contains( command.getRemoveFrom() ) )
    		throw new ProcessingException( "Targets does not contain an entity with id " + command.getRemoveFrom().getValuesMapId() );
