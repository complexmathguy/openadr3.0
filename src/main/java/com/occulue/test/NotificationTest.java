...
public class NotificationTest{
	...
	protected void jumpStart() throws Throwable {
		...
		notificationId = NotificationBusinessDelegate.getNotificationInstance()
	...
	protected void setUpQuerySubscriptions() throws Throwable {
		...
		try {            
			subscriber.notificationSubscribe().updates().subscribe(
					  successValue -> {
						  LOGGER.info(successValue.toString());
						  try {
							  LOGGER.info("GetAll update received for Notification : " + successValue.getNotificationId());
							  if (successValue.getNotificationId().equals(notificationId)) {
								  if (testingStep.equals("create")) {
									  ...
									  state( getAll().size() == sizeOfNotificationList - 1 , "value not deleted from list");
									  LOGGER.info("**********************************************************");
			...
			subscriber.notificationSubscribe( notificationId ).updates().subscribe(
					  successValue -> {
						  LOGGER.info(successValue.toString());
						  try {
							  LOGGER.info("GetOne update received for Notification : " + successValue.getNotificationId() + " in step " + testingStep);
							  testingStep = "delete";
	...
	protected Notification read() throws Throwable {
		...
		try {
			entity = NotificationBusinessDelegate.getNotificationInstance().getNotification( fetchOneSummary );
	...
	protected void update() throws Throwable {
		...
		try {            
			...
			LOGGER.warning( unexpectedErrorMsg );
			LOGGER.warning( msg.toString() + " : primarykey = " + notificationId + " : command -" +  command + " : " + e );
	...
	protected void delete() throws Throwable {
		...
		try{
			NotificationBusinessDelegate.getNotificationInstance().delete( new DeleteNotificationCommand( entity.getNotificationId() ) ).get();
	...
	protected CreateNotificationCommand generateNewCommand() {
        CreateNotificationCommand command = new CreateNotificationCommand( null,  new ValuesMap(),  ObjectType.values()[0],  Operation.values()[0] );
	...
	protected UpdateNotificationCommand generateUpdateCommand() {
	        UpdateNotificationCommand command = new UpdateNotificationCommand( null,  new ValuesMap(),  ObjectType.values()[0],  Operation.values()[0],  null );
