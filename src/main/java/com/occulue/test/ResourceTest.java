...
public class ResourceTest{
	...
	protected void jumpStart() throws Throwable {
		...
		resourceId = ResourceBusinessDelegate.getResourceInstance()
		.createResource( generateNewCommand() )
		.get();
	...
	protected void setUpQuerySubscriptions() throws Throwable {
		...
		try {            
			subscriber.resourceSubscribe().updates().subscribe(
					  successValue -> {
						  LOGGER.info(successValue.toString());
						  try {
							  LOGGER.info("GetAll update received for Resource : " + successValue.getResourceId());
							  if (successValue.getResourceId().equals(resourceId)) {
								  if (testingStep.equals("create")) {
									  ...
									  state( getAll().size() == sizeOfResourceList - 1 , "value not deleted from list");
									  LOGGER.info("**********************************************************");
			...
			subscriber.resourceSubscribe( resourceId ).updates().subscribe(
					  successValue -> {
						  LOGGER.info(successValue.toString());
						  try {
							  LOGGER.info("GetOne update received for Resource : " + successValue.getResourceId() + " in step " + testingStep);
							  testingStep = "delete";
	...
	protected void update() throws Throwable {
		...
		try {            
			...
			LOGGER.warning( unexpectedErrorMsg );
			LOGGER.warning( msg.toString() + " : primarykey = " + resourceId + " : command -" +  command + " : " + e );
	...
	protected void delete() throws Throwable {
		...
		try{
			ResourceBusinessDelegate.getResourceInstance().delete( new DeleteResourceCommand( entity.getResourceId() ) ).get();
	...
	protected CreateResourceCommand generateNewCommand() {
        CreateResourceCommand command = new CreateResourceCommand( null,   new Date(),   new Date(),  org.apache.commons.lang3.RandomStringUtils.randomAlphanumeric(16),  new HashSet<>(),  new HashSet<>(),  ObjectType.values()[0] );
	...
	protected UpdateResourceCommand generateUpdateCommand() {
	        UpdateResourceCommand command = new UpdateResourceCommand( null,   new Date(),   new Date(),  org.apache.commons.lang3.RandomStringUtils.randomAlphanumeric(16),  null,  new HashSet<>(),  new HashSet<>(),  ObjectType.values()[0] );
