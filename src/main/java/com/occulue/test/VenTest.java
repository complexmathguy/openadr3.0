...
public class VenTest{
	...
	protected void jumpStart() throws Throwable {
		...
		venId = VenBusinessDelegate.getVenInstance()
		.createVen( generateNewCommand() )
		.get();
		...
		VenBusinessDelegate.getVenInstance()
				.createVen( generateNewCommand() )
				.get();
	...
	protected void setUpQuerySubscriptions() throws Throwable {
		...
		try {            
			subscriber.venSubscribe().updates().subscribe(
					  successValue -> {
			...
			subscriber.venSubscribe( venId ).updates().subscribe(
					  successValue -> {
						  LOGGER.info(successValue.toString());
						  try {
							  LOGGER.info("GetOne update received for Ven : " + successValue.getVenId() + " in step " + testingStep);
							  testingStep = "delete";
	...
	protected void update() throws Throwable {
		...
		try {            
			...
			LOGGER.warning( unexpectedErrorMsg );
			LOGGER.warning( msg.toString() + " : primarykey = " + venId + " : command -" +  command + " : " + e );
	...
	protected CreateVenCommand generateNewCommand() {
        CreateVenCommand command = new CreateVenCommand( null,   new Date(),   new Date(),  org.apache.commons.lang3.RandomStringUtils.randomAlphanumeric(16),  new HashSet<>(),  new HashSet<>(),  ObjectType.values()[0] );
	...
	protected UpdateVenCommand generateUpdateCommand() {
	        UpdateVenCommand command = new UpdateVenCommand( null,   new Date(),   new Date(),  org.apache.commons.lang3.RandomStringUtils.randomAlphanumeric(16),  new HashSet<>(),  new HashSet<>(),  new HashSet<>(),  ObjectType.values()[0] );
