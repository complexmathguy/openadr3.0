...
public class IntervalTest{
	...
	protected void jumpStart() throws Throwable {
		...
		intervalId = IntervalBusinessDelegate.getIntervalInstance()
		.createInterval( generateNewCommand() )
		.get();
	...
	protected void setUpQuerySubscriptions() throws Throwable {
		...
		try {            
			subscriber.intervalSubscribe().updates().subscribe(
					  successValue -> {
						  LOGGER.info(successValue.toString());
						  try {
							  LOGGER.info("GetAll update received for Interval : " + successValue.getIntervalId());
							  if (successValue.getIntervalId().equals(intervalId)) {
								  if (testingStep.equals("create")) {
									  ...
									  state( getAll().size() == sizeOfIntervalList - 1 , "value not deleted from list");
									  LOGGER.info("**********************************************************");
			...
			subscriber.intervalSubscribe( intervalId ).updates().subscribe(
					  successValue -> {
						  LOGGER.info(successValue.toString());
						  try {
							  LOGGER.info("GetOne update received for Interval : " + successValue.getIntervalId() + " in step " + testingStep);
							  testingStep = "delete";
	...
	protected void update() throws Throwable {
		...
		try {            
			...
			LOGGER.warning( unexpectedErrorMsg );
			LOGGER.warning( msg.toString() + " : primarykey = " + intervalId + " : command -" +  command + " : " + e );
	...
	protected void delete() throws Throwable {
		...
		try{
			IntervalBusinessDelegate.getIntervalInstance().delete( new DeleteIntervalCommand( entity.getIntervalId() ) ).get();
	...
	protected CreateIntervalCommand generateNewCommand() {
        CreateIntervalCommand command = new CreateIntervalCommand( null,  new HashSet<>() );
