...
public class NotificationCommandRestController extends BaseSpringRestController {
     ...
     */
	@PostMapping("/create")
    public CompletableFuture<UUID> create( @RequestBody(required=true) CreateNotificationCommand command ) {
		CompletableFuture<UUID> completableFuture = null;
		try {       
        	
			completableFuture = NotificationBusinessDelegate.getNotificationInstance().createNotification( command );
        }
        catch( Throwable exc ) {
     ...
     */
	@PutMapping("/update")
    public CompletableFuture<Void> update( @RequestBody(required=true) UpdateNotificationCommand command ) {
		CompletableFuture<Void> completableFuture = null;
		try {                        	        
			// -----------------------------------------------
			// delegate the UpdateNotificationCommand
			// -----------------------------------------------
			completableFuture = NotificationBusinessDelegate.getNotificationInstance().updateNotification(command);;
	    }
	    catch( Throwable exc ) {
	    	LOGGER.log( Level.WARNING, "NotificationController:update() - successfully update Notification - " + exc.getMessage());        	
	    }		
    ...
    public CompletableFuture<Void> delete( @RequestParam(required=true) UUID notificationId  ) {
    	...
    	try {
        	NotificationBusinessDelegate delegate = NotificationBusinessDelegate.getNotificationInstance();

        	completableFuture = delegate.delete( command );
    		LOGGER.log( Level.WARNING, "Successfully deleted Notification with key " + command.getNotificationId() );
        }
        catch( Throwable exc ) {
	...
	public void unAssignNotifier( @RequestBody(required=true)  UnAssignNotifierFromNotificationCommand command ) {
		try {
...
//************************************************************************
    ...
    private static final Logger LOGGER = Logger.getLogger(NotificationCommandRestController.class.getName());
